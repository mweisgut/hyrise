...
std::unordered_map<std::string, BenchmarkTableInfo> TPCHTableGenerator::generate() {
  ...
  TableBuilder lineitem_builder{lineitem_chunk_size, lineitem_column_types, lineitem_column_names,
                                order_count * 4};
...
AbstractTableGenerator::IndexesByTable TPCHTableGenerator::_indexes_by_table() const {
  return {
      ...
      {"partsupp", {{"ps_partkey"}, {"ps_suppkey"}, {"ps_suppkey"}}},
      //multi column indexes: {"partsupp", {{"ps_partkey", "ps_suppkey"}, {"ps_suppkey"}}},  // ps_partkey is subset of {ps_partkey, ps_suppkey}
      {"customer", {{"c_custkey"}, {"c_nationkey"}}},
      {"orders", {{"o_orderkey"}, {"o_custkey"}}},
      {"lineitem", {{"l_orderkey"}, {"l_linenumber"}, {"l_partkey"}, {"l_suppkey"}}},
      //multi column indexes: {"lineitem", {{"l_orderkey", "l_linenumber"}, {"l_partkey", "l_suppkey"}}},
